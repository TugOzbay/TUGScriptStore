...
# Version 1.5 Adapted script to take account of new 4 part client names, plus alert level change for 80% subscription
# Version 1.6 (FG) Added IDN and DIRECTFEED only test to stop profiles expanding out of control.
# Version 1.7 (BD) Modified entities subscribed check for RHP clients to support the Entry Level TREP offering where there may be more than one site per DACS
# Version 1.8 (FG) Found a bug whereby the variable vendorService was being referenced outside the original for loop.
# Version 1.9 (BD) Found bug where DELAYFEED permissionings were being set against radmin. Fixed by replacing $service with $vendorService at line 93
...
$reportFile = "./fullyPermRadminUser.log";
...
# Lets check to see if radmin user exists!

if (!(&findUserRecord("TRHS","radmin") == 1))
{
   pp ("WARNING: User radmin does not have a valid DACS account on site TRHS\n");
   #next;
   exit 0;
}

pp ("INFO: User radmin has a valid DACS account checking permissions ...\n");

foreach $vendorService (@vendorServices)
{

        if (grep /IDN|^DIRECTFEED/, $vendorService)
        {
                $vendorPermed=&findServiceRecord($vendorService);;
                if ($vendorPermed == 0)
                {
                        pp("ADDITION:Permissioning radmin for $vendorService on $host at $dbDate\n");
                        &activateUserService("TRHS","radmin",$vendorService,$dbDate);
                        $autoDistribute="True";
                }
                else
                {
                        #pp("INFO:radmin user already permissioned for $vendorService on $host\n");
...
foreach $siteAEarray (@siteAes)
{
   ...
   $aePermed=&findAERecord("TRHS","radmin",$aeId);
        ...
        foreach $vendorService (@vendorServices)
        {
           ...
           if ($aePermed == 0)
           {
                  pp("ADDITION:Permissioning radmin for ae $aeName from service $vendorService on $host at $dbDate\n");
                  &activateUserAE("TRHS","radmin",$aeId,$vendorService,$aeName,$dbDate);
                  $autoDistribute="True";
           }
           else
           {
                  #pp("INFO:radmin user already permissioned for ae $aeName from $vendorService on $host\n");
           }
                }
        }
}
...
if ( $targetType eq "P" )
{
   ...
   foreach my $site (@sites) {
      $numSubscribed=&getNumSubscribed($site);
      $percent =($numEntities ? int($numSubscribed/$numEntities*100) : undef);
      if ( $percent > 80 )
      {
         pp ("ERROR: Site $site is a platform client and has $numSubscribed entities subscribed which is $percent % of the total $numEntities which is deemed a too high. Please consult with the TRHS product owner\n");
      }
   }
}
...
{
   my (@sites,$data);
   $dbh->dbcmd("select distinct site from dacs_main..siteAe\n");
   $dbh->dbsqlexec;
   $dbh->dbresults;
   while($data = $dbh->dbnextrow)
   {
