...
do
        ...
        if [ `df -kl|grep "$disk"|awk '{print $5}'|grep -v Capacity|sed 's/.$//'` -lt ${Pct} ]
         then
                #echo "checking ...     $disk " >> /tmp/FScheck.log
                echo "All ok - less than $Pct % usage on $disk on $DT"
                echo "All ok - less than $Pct % usage on $disk on $DT" >> /tmp/FScheck.log
                echo "------------------------------------------------------------------------------------------  " >> /tmp/FScheck.log
                # $MLDR/sendEmail -s smtp:25 -o message-content-type=text -f $SEND -t $RECV -u \"$HOSTN disk space\" -m \"$output\"
         fi

        if [ `df -kl|grep "$disk"|awk '{print $5}'|grep -v Capacity|sed 's/.$//'` -gt ${Pct} ]
         then
                ...
                # echo "checking ...    $disk " >> /tmp/FScheck.log
                echo "PLS CHECK ASAP - over $Pct % disk usage on $disk  " >> /tmp/FScheck.log
                echo "The $disk with a $diskSpace needs clearing up on $DT" >> /tmp/FScheck.log
                echo "------------------------------------------------------------------------------------------  " >> /tmp/FScheck.log
